...
import { ElementRef, EventEmitter, Inject, Injectable, PLATFORM_ID } from '@angular/core';
...
export class PointerService {
  ...
  private _cannotPanMoreTest(e: PointerEvent, dirX: number): boolean {
    ...
    if (
      ...
      Math.round(this._originalState.aw - bgSize - xPos) < 0
    ) {
  ...
  private _newBGSizeConstraint(newSize: number): number {
    if (this._originalState.widthBound) {
      return newSize < this._originalState.aw
        ? this._originalState.aw
        : newSize;
    } else {
      return newSize / this._originalState.ar < this._originalState.ah
        ? this._originalState.ah * this._originalState.ar
        : newSize;
    }
  }
