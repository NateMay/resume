 ...
 // DEPENDS ON (time-picker) then TEST (time-picker)
  ...
  setDefaultTime(time: string): void {
    ...
    if (defaultTime) {
      ...
      const hour = formatHourByPeriod(defaultTime.getHours(), period as TimePeriod)
      const minutes = defaultTime.getMinutes()
      this.selectedHour = { ...DEFAULT_HOUR, time: hour, angle: hour * 30 };
      this.selectedMinute = { ...DEFAULT_MINUTE, time: minutes, angle: minutes * 6 };
...
function formatHourByPeriod(hour: number, period: TimePeriod): number {
  switch (period) {
    case TimePeriod.AM: return hour === 0 ? 12 : hour;
    case TimePeriod.PM: return hour === 12 ? 12 : hour - 12;
    default: return hour;
  }
}
